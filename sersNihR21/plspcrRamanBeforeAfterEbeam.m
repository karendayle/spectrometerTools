...
function [spectra, analyteArr] = getHITCRamanSpectra(analyteChoice, datasetChoice)  
    ...
    % boosts the signal of the blank 
    xRef = analyteMaxPeakLocation(analyteChoice);
    % Revert back - TO DO check this desired for e-beam case
    xRef = 0;
...
% Used when data collected with KDK scripts
function [wn, rs] = readTXT(thisfilename)
    global numPoints
...
function [wn b] = addOneSpectrum(dir_to_search, filename, xRef)
    ...
    dinfo = dir(txtpattern); % pb for myOption = 4, this matches
    % all avg*.txt files in dir
...
% used only by myOption = 4
function [wn b] = addOneAvgSpectrum(dir_to_search, filename, xRef, K)
    global numPoints
    global waveNumbers
    global myOption
    ...
    [wn, an] = readTXT(strcat(dir_to_search,dinfo(K).name));

%     waveNumbers = wn; % 2021/03/10 needed for option 3
    % 1. Correct the baseline BEFORE calculating denominator + normalizing
    % Returns trend as 'e' and baseline corrected signal as 'f'
    % WHY DOES THIS NEED TRANSPOSING FOR myOption = 4?
    [e, f] = correctBaseline(an');
    % 2. Ratiometric
    % NEW 10/4/18: Calculate the denominator using a window of 0 - 5 points
    % on either side of refWaveNumber. This maps to: 1 - 11 total
...
function c = analyteMaxPeakLocation(analyteChoice)
    switch analyteChoice
        ...
        case 11
            % HITC
            c = 430; % the index of peak with Raman shift ~= 1115
        otherwise
