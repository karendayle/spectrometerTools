...
% Details for the figure caption
% 5 second integration time per acq
% Each spectra average of 5 acqs
% Normalized using 5 points around ref peak
% Displaying average spectrum at each pH level
...
global dirStem
dirStem = "H:\Documents\Data\Made by Sureyya\PolyHEMA\study 2 reducing power AGAIN\";
%dirStem = "Z:\Documents\Data\Made by Sureyya\PolyHEMA\study 2 reducing power AGAIN\"; % Analyzing using remote Matlab client
subDirStem1 = "2 0.375";
subDirStem2 = "6 0.363";
subDirStem3 = "3 0.35";
subDirStem4 = "5 0.338";
...
deltaY = 0.1;
%deltaX = 55;
text(x, y, '48.75 mW', 'Color', black, 'FontSize', myTextFont);
text(x, y, '________', 'Color', black, 'FontSize', myTextFont);
y = y - deltaY;
text(x, y, '41.43 mW', 'Color', red, 'FontSize', myTextFont);
text(x, y, '________', 'Color', red, 'FontSize', myTextFont);
%text(x + deltaX, y, '5 second integration time per acq', 'FontSize', myTextFont);
y = y - deltaY;
text(x, y, '33.5 mW', 'Color', green, 'FontSize', myTextFont);
text(x, y, '________', 'Color', green);
%text(x + deltaX, y, 'Each spectra average of 5 acqs', 'FontSize', myTextFont);
y = y - deltaY;
text(x, y, '26.18 mW', 'Color', ciel, 'FontSize', myTextFont);
text(x, y, '________', 'Color', ciel);
%text(x + deltaX, y, 'Normalized using 5 points around ref peak', 'FontSize', myTextFont);
y = y - deltaY;
text(x, y, '18.25 mW', 'Color', blue, 'FontSize', myTextFont);
text(x, y, '________', 'Color', blue);
%text(x + deltaX, y, 'Displaying average spectrum at each pH level', 'FontSize', myTextFont);

hold off
title('54 nm spheres in polyHEMA gel 1 in flowcell in pH7 buffer at various laser powers', 'FontSize', myFont);
...
function g = myPlot(subDirStem, myColor)
    ...
    if numberOfSpectra > 0
        ...
        plot(thisdata(1,offset:end), normalized(offset:end), 'Color', myColor, 'LineWidth', lineThickness);
