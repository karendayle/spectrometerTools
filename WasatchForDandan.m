...
%myRawData = zeros(1, numPoints, 'double');
...
if (myAns1 ~= 4)
    ...
    while 1
        fprintf("\n2. Taking spectrum...");
        %myRawData = zeros(1, numPoints, 'double'); % HELP. IS THIS NECESSARY?
        myRawData = takeSpectrum(numPoints, spectrometer, integrationTimeMS);
        for i=1:numPoints
            fprintf('myRawData(%d)=%d\n', i, myRawData(i));
        end
        rawFilename = writeSpectrumToFile(pixels, x, myRawData, rawStem);
        ...
        for i = 1:pixels
            spectrumData(i) = myRawData(i) - darkData(i);
        end
        ...
        plotStatus = plotSpectrum(firstTime, xMin, xMax, yMin, yMax, ...
            wavelengths, x, darkData, myRawData, spectrumData, difference, spectrumFilename);
...
% local function to graph the spectrum
    figure
    subplot(2,2,1)
    %plot(wavelengths, rawSpectrum, 'green');
